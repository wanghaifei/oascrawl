...
class Mongo_db {
	...
	*	$operator : $set, $addToSet, $pop, $push, $pushAll, $pull, $pullAll...
	*
	*	@usage: $this->mongo_db->update('foo', $data = array());
	*/
	
	public function update($collection = "", $data = array(), $options = array(), $operator='$set')
	{
		if (empty($collection))
		{
		...
		try
		{
			...
			$this->db->{$collection}->update($this->wheres, array($operator => $data), $options);
			$this->_clear();
			return (TRUE);
		}
		catch (MongoCursorException $e)
	...
	*/
	 ...
	 * @author Me
	 */
	function switch_server($arr)
	{
		$this->connection_string_manual($arr);
		$this->connect();
	}
	
	//$arr: 服务端参数，详见下面代码
	private function connection_string_manual($arr) 
	{
		$this->host	= trim($arr['host']);
		$this->port = trim($arr['port']);
		$this->user = trim($arr['user']);
		$this->pass = trim($arr['pass']);
		$this->dbname = trim($arr['db']);
		$this->persist = trim($arr['persist']);
		$this->persist_key = trim($arr['persist_key']);
		$this->query_safety = trim($arr['query_safety']);
